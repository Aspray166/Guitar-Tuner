...
TIM_OC_InitTypeDef TIM4_CH4Handler;     //定时器3通道4句柄
...
{ 
    ...
    TIM4_CH4Handler.Pulse=arr/2;            //设置比较值,此值用来确定占空比，
                                            //默认比较值为自动重装载值的一半,即占空比为50%
    TIM4_CH4Handler.OCPolarity=TIM_OCPOLARITY_LOW; //输出比较极性为低 
    HAL_TIM_PWM_ConfigChannel(&TIM4_Handler,&TIM4_CH4Handler,TIM_CHANNEL_4);//配置TIM3通道4
...
void HAL_TIM_PWM_MspInit(TIM_HandleTypeDef *htim)
{
	...
	GPIO_Initure.Pin=GPIO_PIN_9;           	//PB1
    GPIO_Initure.Mode=GPIO_MODE_AF_PP;  	//复用推完输出
    GPIO_Initure.Pull=GPIO_PULLUP;          //上拉
    GPIO_Initure.Speed=GPIO_SPEED_HIGH;     //高速
